...
namespace TOK
{
    ...
    int64_t lcg63::compute_nskip(int64_t ns)
    {
        int64_t nskip = ns;
        while (nskip < 0LL)
        {
    ...
    uint64_t lcg63::skip(int64_t ns, uint64_t seed)
    {
        ...
        uint64_t g = mult; // original multiplicative constant
        uint64_t c = add;  // original additive constant

        uint64_t g_next = 1ULL; // new effective multiplicative constant
